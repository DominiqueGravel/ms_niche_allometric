...
par(mfrow=c(2,1), mar=c(3,6,3,1))
	image(x=c(1:length(BS_med)),y=c(1:length(BS_med)),z=mat_med,xlab = "", ylab = "Prey body size rank",cex.axis = 1.25,cex.lab = 1.5,col=c("white","Black"),xlim=c(25,557),ylim=c(25,557))
	mtext(text="A)",side=3,line=0.5,adj=-0.1,cex=1.5)

par(mar=c(5,6,1,1))
	image(x=c(1:length(BS_med)),y=c(1:length(BS_med)),z=Cor_mat,xlab = "Predator body size rank", ylab = "Prey body size rank",cex.axis = 1.25,cex.lab = 1.5,col=c("white","Black"),xlim=c(25,557),ylim=c(25,557))
	mtext(text="B)",side=3,line=0.5,adj=-0.1,cex=1.5)
dev.copy2pdf(file = "Example_MED.pdf")
#dev.off()
